...
Replaces the Shopware 6 administration dashboard with a **modular, drag-and-drop
widget board**. Each user arranges their own widgets; the layout is persisted
per user in the existing `user_config` entity.
...
| Widget catalogue | `core/widget-registry.ts` — a singleton registry of available widgets. |
