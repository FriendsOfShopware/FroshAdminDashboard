...
| Concern | Location |
| --- | --- |
| Replaces the dashboard page | `module/frosh-admin-dashboard/index.ts` — a `routeMiddleware` swaps the component of the core `sw.dashboard.index` route for `frosh-dashboard-index`. Route name, navigation entry and ACL stay untouched. The page still renders core injection points: `sw-settings-services-dashboard-banner` and `sw-extension-component-section` with `sw-dashboard__before-content` / `sw-dashboard__after-content` so Shopware Services and other extensions keep working. |
...
| Persistence | `core/dashboard-layout.service.ts` — reads/writes the layout via `userConfigService` (`user_config` row, key `frosh-admin-dashboard.layout`). On plugin uninstall (without “keep user data”), these rows are deleted. |
